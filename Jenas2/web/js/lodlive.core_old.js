...
var spot_generator = function() { 
	return s4.url + "/Servlet";
}	
...
// Конструктор браузера RDF для работы с DBaaS	
	function OntotextBrowser(repo) {
		...
		var docImages = [];
		var the_graph = s4.implicit;
		s4.onto = s4.host + the_graph + '#';
		var for_ajax = {};				// данные для асинхронного POST-запроса к DaaS
		for_ajax.data = {};
		...
		var browserMessage  = top.browserMessage ? top.browserMessage : opener.top.browserMessage;
		...
		function guessingEndpoint(uri, onSuccess, onFail) {
			...
			function guessi() {
				...
				var jqXHR = $.ajax(for_ajax);			// асинхронный запрос к DaaS
				jqXHR.done(function(response) {			// успешный ответ от DaaS
...
//					panel.append('<h3 class="sprite"><span>' + toLog.resource + '</span><a class="sprite">&#160;</a></h3>');
					panel.append('<h3 class="sprite"><span>Service is disabled for technical reasons, we apologize.</span><a class="sprite">&#160;</a></h3>');
...
//					Позиционирования карты GMap на Обнинск и проставление метки				
		...
		function addNewDoc(obj, ele, callback, manual_click) {
			var x  = $(obj).attr("rel");
			var z  = $(ele).attr("rel");
			var x_onto = x.indexOf(s4.daas);
			var z_onto = z.indexOf(s4.daas);
						...
						$(this).click(function() {
							...
							function newpage() {	
								...
								jqXHR.done(function(response) {			// успешный ответ от DaaS
									...
									else window.open(s4.host + s4.implicit + '.owl');
								});
							...
							function newpage_fail() {
								if(attempt < max_attempts) {
									...
									window.open(s4.host + s4.implicit + '.owl');
									return;
								}
							}
						});
...
//						if(value[akey].indexOf(s4.daas)>-1) types.push(unescape(value[akey]));
...
//							if(obj.indexOf(s4.dbpedia) > -1) return true;					// пропустить типы из DBpedia
//							console.log('obj = ' + obj);
//							console.log('self.anUri = ' + self.anUri + '\n');
//							if (obj != self.anUri) {
								// Те sameAs uris, которые в действительности не являются sameAs, пропускаем
//								if(prop.indexOf('sameAs')>-1){ 
//									var name = obj.substr(obj.indexOf('#')+1);
//									if(s4.individ_classes[name] != s4.individ_classes[self.thing]) return true;
//								};
...
/*			
	function start_3(firstUri, destBox){
		// Все индивиды и их классы
//		s4.individ_classes = {};	
//		for_ajax.data.query = 'PREFIX my:<' + s4.onto + '> SELECT DISTINCT ?subject ?object WHERE {?subject rdf:type owl:NamedIndividual . ?subject rdf:type ?object .FILTER(!(?object = owl:NamedIndividual || ?object = my:Document || ?object = my:Video || ?object = my:Tutorial || ?object = my:Software || ?object = my:Evaluation_Tool))}';
//		attempts = 0;
//		start_go();
		function start_go_fail() {
			if(attempts < max_attempts) setTimeout(start_go, 99);
			else errorBox(destBox);
		}
		function start_go() {
			++attempts;
			var jqXHR_3 = $.ajax(for_ajax);			// асинхронный запрос к DaaS
			jqXHR_3.done(function(response) {		// успешный ответ от DaaS
				var json = JSON.parse(response);
				json = json['results'];
				if(json) json = json['bindings'];
				if(!json) {
				...
				$.each(json, function(key, value){ 
					var name = value.subject.value;
					name = name.substr(name.indexOf('#')+1);
					var _class = value.object.value;
					_class = _class.substr(_class.indexOf('#')+1);
					if(!s4.individ_classes[name]) s4.individ_classes[name] = '';
					s4.individ_classes[name] += _class;
				});
				attempts = 0;
			});
			jqXHR_3.fail(start_go_fail);
		}
	}
*/		
		this.init = function() {
			var firstUri = s4.host + s4.firstUri;
...
//			Вставка красной кнопочки для закрытия панели
			if(top.options.lodlive.frame){
				...
				$close_btn.bind('mouseup', function(){ close_core(); });
			};
			} catch(e){};
		}
	}
...
//								skip = true;
	...
	$.fn.lodlive = function(method) {
		...
		if(location.search.indexOf(s4.dbpedia) < 0) {
			rdf_browser = new OntotextBrowser(location.toString());
		} else {
			function Browser(){} 
